...
- **`components/ChangelogEntryDetails.tsx`** — new client component. Renders the `<details>`/`<summary>` widget for one changelog entry. On the first `toggle` open event it fetches `/api/changelog?filename=...` and renders the body with `<MarkdownContent>`. Subsequent opens reuse the cached state.

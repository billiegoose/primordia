...
- `WebPreviewPanel` gained two new optional props: `fullHeight` (makes the iframe fill the container vertically with `flex-1` instead of a fixed 600 px height) and `className` (forwarded to the outer wrapper, used to strip the rounded border when mounted in the sidebar).

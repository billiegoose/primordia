...
2. **Mouse:** hover over elements — they get a 2px blue outline highlight and a floating `<Component> selector` label above the outline (blue background, white text, readable against any page background).  Click an element to capture it.
   **Touch (tablet/phone):** drag your finger around to highlight elements under it — the same label tracks the current element.  Hold still for ~600 ms to select the highlighted element.

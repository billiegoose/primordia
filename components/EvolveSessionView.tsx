...
interface EvolveSessionViewProps {
  ...
  /** Wall-clock duration of the Claude run in ms (null if not yet complete). */
  initialDurationMs?: number | null;
  /** Input tokens consumed by the Claude run (null if not yet complete). */
  initialInputTokens?: number | null;
  /** Output tokens generated by the Claude run (null if not yet complete). */
  initialOutputTokens?: number | null;
  /** Total API cost in USD for the Claude run (null if not yet complete). */
...
export default function EvolveSessionView({
  ...
  const [inputTokens, setInputTokens] = useState<number | null>(initialInputTokens);
  const [outputTokens, setOutputTokens] = useState<number | null>(initialOutputTokens);
      ...
      while (true) {
        ...
        for (const line of chunk.split("\n")) {
          ...
          try {
            ...
            if (parsed.durationMs != null) setDurationMs(parsed.durationMs);
            if (parsed.inputTokens != null) setInputTokens(parsed.inputTokens);
            if (parsed.outputTokens != null) setOutputTokens(parsed.outputTokens);
            if (parsed.costUsd != null) setCostUsd(parsed.costUsd);
          } catch {
  ...
  return (
    <main className="flex flex-col w-full max-w-3xl mx-auto px-4 py-6 min-h-dvh">
      ...
      {/* Usage metrics — shown when session is done and we have at least one metric */}
      {(status === "ready" || status === "accepted" || status === "rejected") &&
        (durationMs != null || costUsd != null || inputTokens != null || outputTokens != null) && (
        <div className="mb-6 rounded-lg border border-gray-700 bg-gray-900 text-sm overflow-hidden">
          <div className="px-4 py-2.5 border-b border-gray-800">
            <span className="font-semibold text-xs text-gray-300">📊 Usage</span>
          </div>
          <div className="px-4 py-3 flex flex-wrap gap-x-6 gap-y-2 text-xs text-gray-400">
            {durationMs != null && (
              <span>
            ...
            {costUsd != null && (
              <span>
                ...
                <span className="text-gray-200 font-mono">${costUsd.toFixed(4)}</span>
              </span>
            )}
